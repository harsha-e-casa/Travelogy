...
const Modal = ({
  ...
  useEffect(() => {
    // When modal opens, set the first image
  ...
  // Update selected image based on the current index
  useEffect(() => {
...
export default function ActivitiesDetail4() {
  ...
  useEffect(() => {
        ...
        const response = await axios.post(
          ...
          { id },
          {
            headers: {
              ...
              apikey: "412605c3683c38-96bd-45b6-ae06-02e22a8be1b1", // Replace with your API key
            },
          }
        );
        if (response.data.status.success) {
          setHotelData(response.data.hotel); // Assuming 'hotel' is the object in the response
        } else {
          setError(response.data.errors[0]?.message);
          console.log(response.data.errors[0]?.message); // Set the error message if the hotel is unavailable
        }
      } catch (error) {
    ...
    fetchHotelDetails();
  }, [id]); // Triggered when the 'id' changes
  ...
  const mealinfo = [];
  const rating = hotelData?.rt || 0; // Assuming 'hotelData.rt' is the rating
  ...
  const images = hotelData?.img || []; // Ensure images are available
              ...
              <div className="col-lg-8">
                ...
                <div className="box-banner-activities-detail-4">
                  <div className="image-gallery">
                    <div className="image-row">
                      ...
                      <div className="image-column">
                        <div className="image-row-3">
                          {hotelData?.img
                            ?.filter((image) => image.sz === "Standard")
                            .slice(1, 4)
                            .map((image, index) => (
                    ...
                    {/* <button className="see-all-photos-btn">
                      +

...
export default function BookTicket() {
  ...
  const [travellerInfoV, setTravellerInfoV] = useState<any[]>([]);

  // const fetchFlights = useCallback(async (priceId: string) => {
  //   setLoading(true);
  //   setError(null);
  //   try {
  ...
  //       throw new Error(
  //         `API error: ${data?.status?.httpStatus || "Unknown Status"}`
  //       );
  //     }
  ...
  const fetchFlights = useCallback(async (priceId: string) => {
    ...
    try {
      ...
      // Make API request
      const data: any = await postData("travelogy/one-way/fetch-data", reqData); // `any` here
      ...
      // Access the first trip from tripInfos
      const firstTrip = data?.tripInfos?.[0];

      // If no trip data, throw an error
      if (!firstTrip) {
        throw new Error("No trip data available");
      }

      // Update booking ID, ensuring it's a valid string
      if (data?.bookingId) {
        setBookingId(data.bookingId);
      }

      // Extract segments and segment ID
      const segs = firstTrip?.sI;
      const id = segs?.[0]?.id?.trim(); // Ensure id exists and is trimmed

      if (!segs || !id) {
        throw new Error("No segment or price data returned by the API");
      }

      // Set selected ID and segments
      setSelectedId(id);
      setSegments(segs);

      // Calculate total fare detail safely with fallback to empty object if missing
      const totalFareDetail = data?.totalPriceInfo?.totalFareDetail ?? {};
      ...
      const ssrInfo = segs?.[0]?.ssrInfo;
      const mealOptions = ssrInfo?.MEAL ?? []; // Default to empty array if no meal options

      // Extract fareRuleInformation safely
      const fareRuleInformation = firstTrip?.fareRuleInformation ?? {};

      // Update segment price list
      setSegmentsPrice(firstTrip?.totalPriceList ?? []);

      // Extract cabinClass and paxInfo from searchQuery
      const cabinClass = data?.searchQuery?.cabinClass ?? "Unknown Class";
      console.log("Cabin Class:", cabinClass);

      const paxInfo = data?.searchQuery?.paxInfo ?? {}; // Default to empty object

      // Safely update passenger counts
      if (paxInfo.ADULT) {
        setNumAdults(paxInfo.ADULT);
      }
      ...
      console.error("Error caused:", err);
      setError(
        "Keys passed in the request are already expired. Please pass valid keys."
      );
    } finally {
      setLoading(false);
    }
  ...
  }, [bookingIdget, fetchFlights, tcs_id]);
              ...
              <div className="row mt-20">
                                ...
                                <div className="item-info-flight">
                                  ...
                                  <div className="list-flight-facilities">
                                    <li className="baggage">
                                      ...
                                      : (Adult) Check-in :{" "}
                                      {segmentsPrice[0].fd.ADULT.bI.iB} (01
                                      Piece only)
                                    </li>
                                    <li className="cabin">
                                      <svg
                                      ...
                                      Cabin baggage{" "}
                                      {segmentsPrice[0].fd.ADULT.bI.cB}
                                    </li>
                                    {/* <li className="meal">
                        ...
                        <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                          <div className="sm:col-span-1">
                            <dd className="mt-1 text-sm font-medium text-gray-900">
                              <Avatar
                                className="mr-1"
                                style={{ backgroundColor: "rgb(255 201 84)" }}
                          ...
                          <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                            <div className="sm:col-span-1">
                              <dd className="mt-1 text-sm font-medium text-gray-900">
                                <Avatar
                                  className="mr-1"
                                  style={{ backgroundColor: "rgb(254 99 214)" }}
                                  icon={<UserOutlined className="text-black" />}
                                />{" "}
                                CHILD (2-12 Yrs){" "}
                              </dd>
                ...
                <div className="col-lg-4">
                  <div className="booking-form add_sticky">
                    ...
                    <BookingForm totalpricee={totalpricee} />
                  </div>
                  {/* …side banners… */}
                </div>
              </div>

...
console.log("apiData from mealinfo",apiData)
//     const data={
//     "tripInfos": [
//         {
//             "sI": [
...
console.log("ssrInfo from meal", ssrInfo);
const hasMeal = ssrInfo.some(e => Array.isArray(e.MEAL) && e.MEAL.length > 0);
...
{hasMeal?(<>
 <Form form={form} name="mealForm" layout="vertical" autoComplete="off">
      ...
      {Array.from({ length: numAdults }).map((_, index) => (
        <div className="p-3 flex flex-row gap-5 items-center" key={`adult-${index}`}>
          ...
          <Form.Item name={`adultMeal-${index}`} style={{ marginBottom: 0 }}>
      ...
      {Array.from({ length: numChild }).map((_, index) => (
        <div className="p-3 flex flex-row gap-5 items-center" key={`child-${index}`}>
          ...
          <Form.Item name={`childMeal-${index}`} style={{ marginBottom: 0 }}>
            <Select className="h-10 w-100" placeholder="Add Meal">
              {ssrInfo.flatMap((e) =>
                e.MEAL?.map((bag) => (
                  <Option key={bag.code} value={bag.code}>
 ...
 <div className="p-3 text-sm text-gray-500">No  options available for this flight.</div>
</>)}

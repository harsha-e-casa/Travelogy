...
>
        ...
        {Array.from({ length: numChild }).map((_, index) => (
          <div className="p-2 flex gap-4 items-center" key={`child-${flightIndex}-${index}`}>
            ...
            <Form.Item name={`childBaggage-${flightIndex}-${index}`}  style={{ marginBottom: 0,width:"500px"}}>
...
>
        ...
        {Array.from({ length: numInfants }).map((_, index) => (
          <div className="p-2 flex gap-4 items-center" key={`infant-${flightIndex}-${index}`}>
            ...
            <Form.Item name={`infantBaggage-${flightIndex}-${index}`}  style={{ marginBottom: 0,width:"500px"}}>
              <Select className="h-10 w-100" placeholder="Add Baggage" disabled={baggageOptions.every((bag) => !bag.amount)}
>
                {baggageOptions.map((bag) => (
                  <Option key={bag.code} value={bag.code}  disabled={!bag.amount}>

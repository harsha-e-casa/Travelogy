...
const ExtraBaggage = ({
  ...
  // Pre-fill values from storedTravellerInfos
  useEffect(() => {
    if (!storedTravellerInfos || !Array.isArray(storedTravellerInfos) || !apiData) return;
    ...
    tripInfos.forEach((trip) => {
      ...
      segmentinfo.forEach((segment, flightIndex) => {
        ...
        for (let index = 0; index < numAdults; index++) {
          const traveller = storedTravellerInfos[index];
          if (traveller?.ssrBaggageInfos?.[flightIndex]?.code) {
            values[`adultBaggage-${flightIndex}-${index}`] = `${segment.id}|${traveller.ssrBaggageInfos[flightIndex].code}`;
          }
        }
        ...
        for (let index = 0; index < numChild; index++) {
          const traveller = storedTravellerInfos[numAdults + index];
          if (traveller?.ssrBaggageInfos?.[flightIndex]?.code) {
            values[`childBaggage-${flightIndex}-${index}`] = `${segment.id}|${traveller.ssrBaggageInfos[flightIndex].code}`;
          }
        }
      });
    });
    ...
    form.setFieldsValue(values);
  }, [storedTravellerInfos, apiData, numAdults, numChild, form, tripInfos]);
  ...
  const handleValuesChange = (changedValues, allValues) => {
    ...
    const allBaggage = Object.keys(allValues)
      .filter(
        (key) => key.startsWith("adultBaggage") || key.startsWith("childBaggage")
      )
      .map((key) => allValues[key])
      .filter(Boolean);
                  ...
                  >
                    ...
                    >
                      <Select
                        ...
                        placeholder="Add Baggage"
                        disabled={baggageOptions.every((bag) => !bag.amount)}
                        onChange={(val) => (console.log("baggage changed value ", val))}
                      >
                        {baggageOptions.map((bag) => (
                          <Option
                            key={bag.code}

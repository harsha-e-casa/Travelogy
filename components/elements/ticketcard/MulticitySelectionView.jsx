...
for (let i = 1; i <= 3; i++) { 
  const from = getCookie(`gy_da_str${i}`);
  const to = getCookie(`gy_aa_str${i}`);
  
  if (from && to) {
    cityPairs.push({ from, to });
  }
}
...
const calculateTotalFare = (fd, adultCount, childCount, infantCount, getCookie) => {
  ...
  return {
    ...
    label: (
      <div>
        <p className="text-sm-bold text-gray-600">{pair.from} → {pair.to}</p>
        {travelDate && <p style={{ fontSize: "12px", color: "#888" }}>{travelDate}</p>}
      </div>
    ),
    children: (
      <>
            ...
            <div key={i}>
              <div className="" style={{ paddingBottom: "10px" }}>
                  ...
                  <div
                        ...
                        style={{ width: "unset", top: index === 0 ? "0" : "49%" ,marginBottom:"00px",display:"block"}}
                      >
                        <div className="flex items-center justify-center  ">
                          ...
                          />
                          <div className="text-[10px]">{segment.fD.aI.name}</div>
                        </div>
                      </div>
                    ...
                    >
                      ...
                      <div className="flight-price-1 border-1  price-div flex justify-center items-center flex-col " style={{ width: "245px", paddingLeft: "20px" }}>
                  <Radio.Group
                  ...
                  >
                    {(showAllFares
                      ...
                      const fareValue = calculateTotalFare(e.fd, adultCount, childCount, infantCount, getCookie);
                      return (
                    ...
                    {ticket.totalPriceList.length > 2 && (
                      <button className="view-more-txt" style={{ textAlign: "right", fontSize: "10px" }} onClick={() => setShowAllFares((prev) => !prev)}>
  ...
  onClick={() => {
    ...
    const totalPrice = calculateTotalFare(fareFD, adultCount, childCount, infantCount, getCookie);
    ...
    setSelectedFlights((prev) => {
      const newFlights = { ...prev, [tabIndex]: updatedFlight };
...
>
  ...
  const totalFlightPrice = Object.values(selectedFlights).reduce((sum, flight) => {
    const numericPrice = typeof flight.price === "string" 
  ...
  return (
    <div className=" p-4 rounded bg-black text-white sticky-summary  ">
      <div className="flex items-center justify-between gap-6">
        ...
        {/* Flight Segments */}
        <div className=" flight-scroll pb-2  flex items-center flex-row gap-6 overflow-x-auto whitespace-nowrap " style={{width:"900px"}}>
          {Object.entries(selectedFlights).map(([key, flight], i) => (
            <div     key={i}
            className=""
    style={{
      borderLeft: i === 1||i==2 ? "2px solid white" : "",
      
      padding: "0 12px",
      
    }}
>
                  <div key={i} className="flex items-center gap-5 border-r border-white pr-4">
              ...
              <div className="flex flex-col">
                    <p className="font-semibold text-sm">{flight.flightName}</p>
                    <p>{flight.airlineCode}-{flight.flightNumber}</p>
                </div>
              </div>
            ...
            <div className="flex flex-row justify-end pr-4">
                  {flight.price && (
                  <p className="font-semibold mt-1 text-white">Fare: ₹{flight.price}</p>
                )}
            </div>
            </div>
        ...
        <div className="ml-auto flex flex-row items-center gap-4">
          <div className="text-lg font-bold text-white">
             ₹{new Intl.NumberFormat("en-IN").format(totalFlightPrice)}
          </div>
          <button className=" btn btn-gray btn">
            BOOK
          </button>
        </div>
      </div>

...
export default function ByDepartureTime({
  ...
  const name = `departureTime-${tabIndex}`;
  ...
  const resetPill: React.CSSProperties = {
    display: departureTime !== "all" ? "inline-flex" : "none",
    alignItems: "center",
    gap: 6,
    padding: "6px 10px",
    borderRadius: 999,
  ...
  return (
    <div className="box-collapse scrollFilter">
      {/* Time range cards (horizontal) */}
      <div role="radiogroup" aria-label="Departure time" style={cardsWrap}>
        {TIME_OPTIONS.map((opt, i) => {
          const active = departureTime === opt.value;
          const id = `${name}-${i}`;
          return (
      ...
      <div style={groupWrap}>
        <button
          ...
          aria-label="Clear departure time filter"
        >
          Cancel
